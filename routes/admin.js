...
module.exports = (express, passport)=>{
    ...
    router.post('/image/upload', passport.authenticate('jwt', {session:false}), upload.single('image'),
    (req,res,next)=>{
    ...
    router.get('/product/paginate/:start/:count', 
      ...
      router.get('/cat/all', passport.authenticate('jwt',{session:false}),
      (req,res)=>{
        Cat.all()
          .then(result=>res.json({con:true,msg:result}))
          .catch(err=>res.json({con:false, msg:err}))
